...
def signup():
  ...
  password=data.get('password') #from here we extracted all name , email and password 
  ...
  hashed_password = generate_password_hash(password) #the password is hashed
  ...
  users_collection.insert_one(new_user) #inserted the new user details in the collection
...
def login():
  ...
  user=users_collection.find_one({"email":email}) #it got all the data including the password as well
  if not user:
...
def login_with_google():
  ...
  if existing_user:
    # 1. Generate the fallback username ONLY if the login is successful
    ...
    return jsonify({
      ...
      "user": {
          ...
          "username": existing_user.get("username", fallback_username)
      }}), 200
  else:
