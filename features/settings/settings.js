document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch(`http://localhost:5000/api/settings/${encodeURIComponent(email)}`);
        if (response.ok) {
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const updateRes = await fetch('http://localhost:5000/api/settings/update', {
                    method: 'PUT',
                ...
                if (currentPwd || newPwd || confirmPwd) {
                    if (newPwd !== confirmPwd) {
                        ...
                        const pwdRes = await fetch('http://localhost:5000/api/settings/change-password', {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                email: email,
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener('click', async () => {
            ...
            if (confirm("Are you absolutely sure you want to delete your account? This action cannot be undone.")) {
                try {
                    const response = await fetch('http://localhost:5000/api/settings/delete', {
                        method: 'DELETE',
                        headers: { 'Content-Type': 'application/json' },
